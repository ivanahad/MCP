...
\item INV 2 : La variable "depart" contient l'indice auquel nous avons démarré le tour courant du cadran, et 0 $\le$ depart $\le$ cadran.length

\item INV 3 : La variable "memoire" contient l'élément que nous devons déplacer.

\item INV 4 : La variable "index" contient l'indice auquel nous devons déplacer l'élément contenu dans "mémoire", et 0 $\le$ index $\le$ cadran.length
...
\item \{rotation=$r_{1}$ $\wedge$ cadran=[$e_{0}$,..., $e_{cadran.length-1}$ ]\} t.q. (0$\le$ $r_{1}$ $\le$ cadran.length)\\ 

depart:=0\\

\item \{rotation=$r_{0}$ $\wedge$ cadran=[$e_{0}$,..., $e_{cadran.length-1}$ ] $\wedge$ depart=0\} t.q. (0$\le$ $r_{0}$ $\le$ cadran.length)\\

memoire:=cadran[depart]\\

\item \{rotation=$r_{0}$ $\wedge$ cadran=[$e_{0}$,..., $e_{cadran.length-1}$ ] $\wedge$ depart=0 $\wedge$ memoire=$e_{0}$ \} t.q. (0$\le$ $r_{0}$ $\le$ cadran.length)\\

index:=rotation

\item \{rotation=$r_{0}$ $\wedge$ cadran=[$e_{0}$,..., $e_{cadran.length-1}$ ] $\wedge$ depart=0 $\wedge$ memoire=$e_{0}$ $\wedge$ index=$r_{0}$\} t.q. (0$\le$ $r_{0}$ $\le$ cadran.length)\\

i:=0\\

\item \{rotation=$r_{0}$ $\wedge$ cadran=[$e_{0}$,..., $e_{cadran.length-1}$ ] $\wedge$ depart=0 $\wedge$ memoire=$e_{0}$ $\wedge$ index=$r_{0}$ $\wedge$ i=0\} t.q. (0$\le$ $r_{0}$ $\le$ cadran.length)\\
\end{itemize}
...
\item \{rotation=$r_{1}$ $\wedge$ cadran=[$e_{0}$,..., $e_{cadran.length-1}$ ] $\wedge$ depart=$d_{1}$ $\wedge$ memoire=$m_{1}$ $\wedge$ index=$index_{1}$ $\wedge$ i=$i_{1}$\} t.q. ((0$\le$ $r_{1}$ $\le$ cadran.length $\wedge$ $cadran_{0}$.length=$cadran_{1}$.length  $\wedge$ 0 $\le$ $i_{1}$ $\le$ cadran.length $\wedge$ \textbf{A COMPLETER})= \textbf{Cond1} )\\
...
\{rotation=$r_{1}$ $\wedge$  tmp=$e_{index1}$ $\wedge$ cadran=[$e_{0}$,..., $e_{index1-1}$, $m_{1}$, $e_{index1+1}$, ...,  $e_{cadran.length-1}$ ] $\wedge$ depart=$d_{2}$ $\wedge$ memoire=$e_{index1}$ $\wedge$ index=$index_{2}$ $\wedge$ i=$i_{1}$+1 \} t.q. ((\textbf{Cond1} $\dot{\wedge}$ $index_{1}$=$d_{1}$))\\
